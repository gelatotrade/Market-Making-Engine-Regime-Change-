...
IBKR_NET_REBATE_BPS = 0.01
IBKR_SEC_FEE_BPS    = 0.14
IBKR_FINRA_BPS      = 0.003
NET_COST_PER_FILL   = IBKR_SEC_FEE_BPS + IBKR_FINRA_BPS - IBKR_NET_REBATE_BPS
ADVERSE_SEL         = 0.45

# -- Assets: 10 global ETFs + MAG7 stocks + 3 sector ETFs = 20 assets --
ASSETS = [
...
def run_mm_backtest(opens, highs, lows, closes, params):
    ...
    total_spread = 0.0
    overlay_inv = 0.0
    max_overlay = min(order_sz * n_levels * 2, 0.50)
...
def main():
    ...
    print(f'\n  IBKR fee:  net {NET_COST_PER_FILL:.3f} bps/fill (comm 0.03 - rebate {IBKR_NET_REBATE_BPS:.2f} + SEC {IBKR_SEC_FEE_BPS:.2f})')
...
def plot_equity_curves(df, curves):
    ...
    fig2.suptitle('Market-Maker Backtest -- Alpha and Calmar Summary',
